...
function Validator(form) {
    ...
    const formRules = {
        ...
        requiredIf(selector, selectorValue) {
            return (value) => {
                const input = formElement.querySelector(`[name="${selector}"]:checked`);
                return input && input.value == selectorValue && !value
    ...
    formElement.onsubmit = (e) => {
        ...
        if (enabledInputs) {
            ...
            for (let input of enabledInputs) {
                cancelValidate({ target: input });
                const isValid = handleValidate({ target: input });
    ...
    function cancelValidate(e) {
        const inputGrp = getParent(e.target, ".form-group");
        removeInvalid(inputGrp);
    }

    function removeInvalid(inputGrp) {
    ...
    // Additional function for radioInputs
    const radioInputs = Array.from(
        formElement.querySelectorAll("[name^='AppraisalInfos'][type='radio']")
    );
    for (let input of radioInputs) {
        input.addEventListener("input", (e) => {
            const related = e.target.name.split(".")[0] + ".DetailAnswer";
            const relatedInput = document.querySelector(`[name='${related}']`);

            if (relatedInput) {
                removeInvalid(getParent(relatedInput, ".form-group"));
            };
        });
    }
};
